...
class TrafficProfiler:
    ...
    def detect_anomalies(self, protocol_data=None):
        if not self.profile or self.profile.sample_count == 0:
            return {"anomalies": [], "note": "Aucune baseline — lancez l'apprentissage d'abord"}
            ...
            if ip in self.profile.known_ips:
                ...
                if ip_threshold > 5 and count > ip_threshold:
                    anomalies.append({
                        ...
                        "message": f"Activité anormale {ip}: {count} connexions (baseline: {info['avg_conns']:.0f})",
                        "ip": ip, "current": count,
                ...
                anomalies.append({
                    "type": "unknown_ip",
                    "severity": "low",
                    "message": f"IP inconnue détectée: {ip} ({count} connexions)",
                    "ip": ip, "count": count,
                    "timestamp": now.isoformat(),
                })
